...
class Snake:
  ...
  def has_eaten(self, food):  
    if self.body[0] == food.pos:
      ...
      self.color = "#{:02x}{:02x}{:02x}".format(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))  # Generate a random color
      return True
    return False

  def render(self, game):
      for coord in self.body:
          game.canvas.create_rectangle(coord.x * 32, coord.y * 32, coord.x * 32 + 32, coord.y * 32 + 32, fill=self.color)
